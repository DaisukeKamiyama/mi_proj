...
サイズボックスビュー
*/
class AView_SizeBox : public AView
{
	//コンストラクタ、デストラクタ
  public:
	AView_SizeBox( const AWindowID inWindowID, const AControlID inID );
	virtual ~AView_SizeBox();
  private:
	void					NVIDO_Init();
	...
	//情報取得
  private:
	AViewType				NVIDO_GetViewType() const {return kViewType_SizeBox;}
