...
分割線ビュー
*/
class AView_VSeparator : public AView
{
	//コンストラクタ、デストラクタ
  public:
	AView_VSeparator( const AWindowID inWindowID, const AControlID inID );
	virtual ~AView_VSeparator();
  private:
	void					NVIDO_Init();//#92
	...
	//情報取得
  private:
	AViewType				NVIDO_GetViewType() const {return kViewType_VSeparator;}
  ...
  private:
	...
	ACursorType				EVTDO_GetCursorType( const ALocalPoint& inLocalPoint, const AModifierKeys inModifierKeys ) {return kCursorType_ResizeLeftRight;}
