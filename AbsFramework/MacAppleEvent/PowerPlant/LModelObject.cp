...
// ===========================================================================
//	LModelObject.cp				PowerPlant 2.2.2	ｩ1993-2005 Metrowerks Inc.
// ===========================================================================
//
		...
		case formAbsolutePosition: {
			...
			switch (subSpec) {
				...
				case kAEAny:
					subPosition = 0;
					if (subCount > 0) {
						subPosition = //#1012((UInt16) ::Random() % (UInt16) subCount)
										+ 1;
					}
					break;
...
//Added the following 
//R0129 thru対応
//このコードは下記のIimoriさんのホームページに掲載されたコードを使用させていただきました。
//http://www.bekkoame.ne.jp/~iimori/bugreports/formRange.html
//なお、Getには対応できるが、Set, Selectは未対応。
//GetModelTokenSelf()でoutTokenに、rangeのtokenをリスト形式で格納し、
//その後、リストのアイテムごとに、各種eventが実行される。
//たとえば、リストのアイテムが段落の場合、Getで、リスト形式で段落を取得できるが、
//Selectでは、段落毎にSelectが実行され、結局最後の段落が選択されるのみとなる。
void
LModelObject::GetSubModelByComplexKey(
			DescType        inModelID,
			DescType        inKeyForm,
			...
			AEDesc&         outToken) const
{
	switch ( inKeyForm ) {
	  case formRange: {
			...
			ThrowIfError_(::AECoerceDesc(&inKeyData,typeAERecord,selectionRecord));
			ThrowIfError_(::AEGetKeyDesc(selectionRecord,keyAERangeStart,typeObjectSpecifier,rangeStaSpec));
			ThrowIfError_(LModelDirector::Resolve(rangeStaSpec,rangeStaToken));
			ThrowIfError_(::AEGetKeyDesc(selectionRecord,keyAERangeStop,typeObjectSpecifier,rangeEndSpec));
			ThrowIfError_(LModelDirector::Resolve(rangeEndSpec,rangeEndToken));
			LModelObject *staObj = GetModelFromToken(rangeStaToken);
			LModelObject *endObj = GetModelFromToken(rangeEndToken);
			//----------------------------------------------------------------------
			//  Objectの継承関係がある場合、直接の ModelKindは inModelIDに
			//  一致しない場合があるので、この checkは不要みたい.
			//----------------------------------------------------------------------
			//          if (   (staObj->GetModelKind() != inModelID)
			//              or (endObj->GetModelKind() != inModelID)) {
			//              Throw_Err(errAENoSuchObject);
			//          }
			//----------------------------------------------------------------------
			//  Finderは逆順でも受付けるので、それに習います.
			//----------------------------------------------------------------------
			SInt32 staIdx = GetPositionOfSubModel(inModelID,staObj);
			SInt32 endIdx = GetPositionOfSubModel(inModelID,endObj);
			if (endIdx < staIdx) {
				SInt32 tmp = staIdx;
				staIdx = endIdx;
				endIdx = tmp;
...
LModelObject::HandleAppleEventAll(
	...
	AEDesc				&outResult,
	//Modified by daisuke for mi (http://www.mimikaki.net) 2014/8/24 from HERE
	/*#1034 long*/SInt32				inAENumber)
//Modified by daisuke for mi (http://www.mimikaki.net) 2014/8/24 to HERE
{
	bool	handled = false;
	...
	AppleEvent			&outResult)
{
	...
	//Modified by daisuke for mi (http://www.mimikaki.net) 2014/8/24 from HERE
	UAEDesc::AddPtr(&outResult, 0, /*#1034 typeLongInteger*/typeSInt32, &count, sizeof(count));
	//Modified by daisuke for mi (http://www.mimikaki.net) 2014/8/24 to HERE
}
