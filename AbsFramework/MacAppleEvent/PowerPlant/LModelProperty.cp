	...
	//Modified by daisuke for mi (http://www.mimikaki.net) 2014/8/24 to HERE
{
	...
	if (inAENumber == ae_GetDataSize) {
		...
		//Modified by daisuke for mi (http://www.mimikaki.net) 2014/8/24 from HERE
		OSErr err = ::AECreateDesc(/*#1034 typeLongInteger*/typeSInt32, &theSize, sizeof(SInt32),
								&outResult);
		//Modified by daisuke for mi (http://www.mimikaki.net) 2014/8/24 to HERE
		ThrowIfOSErr_(err);
	}
...
//Added the following 6 lines
	//Unicode—Dæ
	if( ::AEGetParamPtr(&inAppleEvent,keyAEData, 'utxt', nil,nil,0,nil) == noErr )
	{
		value.GetParamDesc(inAppleEvent, keyAEData, 'utxt');
	}
	else
//Modified by daisuke for mi (http://www.mimikaki.net) 2009/2/15 to HERE
