...
//Comment-out the following 12 lines
...
struct SAETableRec {					// Format of an 'aedt' entry
	OSType	eventClass;
	OSType	eventID;
	//Modified by daisuke for mi (http://www.mimikaki.net) 2014/9/6 from HERE
	/*#1034 long*/SInt32	refCon;
	//Modified by daisuke for mi (http://www.mimikaki.net) 2014/9/6 to HERE
};
...
//Added the following 1 line
	SInt16	numTables = ::CountResources('aedt');
//Modified by daisuke for mi (http://www.mimikaki.net) 2009/2/15 to HERE
...
//Added the following 1 line
		Handle tableH = ::GetIndResource('aedt', i);
//Modified by daisuke for mi (http://www.mimikaki.net) 2009/2/15 to HERE
	...
	SInt32			inItemCount)
{
	...
	switch (position) {
		...
		case kAEAny:					// Pick a random item number
			index = 0;
			if (inItemCount > 0) {
				index = /*#1012((UInt16) ::Random() % (UInt16) inItemCount) +*/ 1;
			}
			break;
	...
	SInt32		inInt32)
{
	...
	//Modified by daisuke for mi (http://www.mimikaki.net) 2014/9/6 from HERE
	OSErr	err = ::AECreateDesc(/*#1034 typeLongInteger*/typeSInt32, &inInt32, sizeof(inInt32),
									&mDesc);
	//Modified by daisuke for mi (http://www.mimikaki.net) 2014/9/6 to HERE
	ThrowIfOSErr_(err);
